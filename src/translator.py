...
def TranslateNodeToJava(node):
  if node is None:
    ...
    return 'Obj.X(%f)' % str(node)
  elif type(node) == str:
    return 'Obj.X(%s)' % SanitizeStringForJava(node)
  elif type(node) == list:
    return 'Obj.A(%s)' % ', '.join(map(TranslateNodeToJava, node))
  elif type(node) == dict:
    return 'Obj.D(%s)' % ', '.join('%s, %s' % tuple(map(TranslateNodeToJava, (k, v))) for k, v in node.items())
  elif type(node) == Origin:
    return 'Obj.D(Obj.X("filespec"), FILESPEC, Obj.X("source"), SOURCE, Obj.X("position"), Obj.X(%d))' % node.position
  else:
...
def TranslateModuleToJava(filespec, string):
  ...
  return r"""
    {
      final Obj FILESPEC = Obj.X(%s);
      final Obj SOURCE = Obj.X(%s);
