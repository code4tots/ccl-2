...
Translate CCL to javascript.

Seeing as so much of javascript is semantically similar to CCL that
this might be the easiest implementation.

However, I am also considering some semantic changes as well.
...
function str(s) {
  str = Object.create(String)
  // TODO
}

function L() {
  arguments // TODO: Create a list from this.
}
...
""" % (
  ...
  def visitModule(self, node):
    return "{name}: function(scope) {{ {body} }}".format(
        name=node.name,
  ...
  def visitFunctionCallExpression(self, node):
    vararg = 'L()' if node.vararg is None else self.visit(node.vararg)
    if node.vararg is None:
      return '%s(%s)' %
    else:
      return '%s.__call__(L(%s).extend(%s))' % (
        self.visit(node.f),
        ', '.join(self.visit(a) for a in node.args),
        vararg)

  def visitStringExpression(self, node):
    return 'str(' + sanitize_string(node.val) + ')'

  def visitNameExpression(self, node):
    return 'scope.%s' % node.name
